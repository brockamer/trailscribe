...
- **Model:** `anthropic/claude-sonnet-5` (OpenRouter format `<provider>/<model>`). Upgraded from `anthropic/claude-sonnet-4-6` 2026-09-11 — operator call, ~33% cheaper both directions per OpenRouter's live pricing at the time. Swapped back from the brief `openai/gpt-5-mini` direction (2026-04-22 → 2026-04-25) after P1-21 burn-in proved gpt-5-mini is a *reasoning model* that consumes all completion tokens on chain-of-thought before producing the JSON output — caused empty-content failures on shorter prompts (no-GPS edge case). Sonnet 4.6 was a non-reasoning model: structured JSON output was reliable within the configured `max_tokens` budget (currently 600), no chain-of-thought token consumption. Sonnet 5's structured-output behavior on this seam is not yet burn-in verified — it is a drop-in id swap through OpenRouter's OpenAI-compatible endpoint, but the gpt-5-mini failure above was exactly this seam, so watch the first production `!post` and `!track` narratives for empty-content errors.
...
- **Amendment (2026-09-11):** Upgraded to `anthropic/claude-sonnet-5` — operator call. Cost per 1K re-pinned in `wrangler.toml`: `0.002` input, `0.010` output (OpenRouter live pricing verified 2026-09-11). Note: the `0.00015`/`0.0006` figures in the paragraph above were already stale before this change — they don't match the `0.003`/`0.015` that was actually live in `wrangler.toml`; left as-is rather than silently corrected.
