...
- `MAPSHARE_KEY` — per-tenant MapShare slug (path component appended to `MAPSHARE_BASE`); `HTS69RDNH2` (the auto-generated address) for prod and staging since 2026-09-13. The custom name `trailscribe` was lost on 2026-09-12 and Garmin now reports it as "not available", so it cannot be re-claimed. **If the feed answers HTTP 302 with the correct access code (a wrong code still gets 401), the MapShare toggle is OFF** in Garmin Explore → MapShare on the device user's account (`trailscribeapp@gmail.com`), not a bad secret — turn it on and accept the consent prompt. Seen twice on 2026-09-13.
